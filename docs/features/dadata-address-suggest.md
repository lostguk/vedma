...
При оформлении заказа бэкенд дополнительно проверяет, что выбранный адрес доставляемый: `fias_level >= 8` (дом) либо населённый пункт (`city` / `settlement`) + дом; в городе нужна улица.
